...
import {
  ...
  ActivityIndicator, Alert, Modal, FlatList, TouchableOpacity,
  Platform,
} from "react-native";
...
// ── Voice Slider ───────────────────────────────────────────────────────────────
function SliderRow({
  label, value, min, max, step, hint, onChange, onReset,
}: {
...
export default function HomeScreen() {
  ...
  const runSingle = useCallback(async () => {
    ...
    try {
      ...
      setSingleStatus({ msg: "Done — tap Play to listen or Share to save", type: "success" });
      await playAudio(uri);
    } catch (err: any) {
      setSingleStatus({ msg: "Error: " + err.message, type: "error" });
    }
  ...
  const runBatch = useCallback(async () => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      try {
        ...
        if (batchMode === "translate") {
          speakText = await translateText(text, batchSourceLang, tgtLang);
        }
    ...
    if (filePaths.length > 0) {
      try {
        setBatchStatus({ msg: "Zipping files for sharing...", type: "active" });
        // Share the first file as a sample if only one, otherwise share folder
        if (filePaths.length === 1) {
          await Sharing.shareAsync(filePaths[0], { mimeType: "audio/mpeg", dialogTitle: "Save MP3" });
        } else {
          // Share each file via share sheet (iOS Files app will collect them)
          await Sharing.shareAsync(filePaths[0], { mimeType: "audio/mpeg", dialogTitle: `Save ${filePaths.length} MP3 files` });
        }
      } catch (e) {
    ...
    if (errors.length === 0) {
      ...
      setBatchStatus({ msg: `Done — ${lines.length - errors.length} OK, ${errors.length} failed`, type: "error" });
    }
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]}>
      ...
      <View style={styles.tabBar}>
        <Pressable style={[styles.tabBtn, activeTab === "single" && styles.tabBtnActive]}
        ...
        <Pressable style={[styles.tabBtn, activeTab === "batch" && styles.tabBtnActive]}
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
        ...
        {activeTab === "single" && (
          <View style={styles.section}>
            ...
            <View style={styles.modeRow}>
              <Pressable style={[styles.modeBtn, singleMode === "translate" && styles.modeBtnActive]}
                onPress={() => setSingleMode("translate")}>
                <Text style={[styles.modeBtnText, singleMode === "translate" && styles.modeBtnTextActive]}>🌐 Translate</Text>
              </Pressable>
              <Pressable style={[styles.modeBtn, singleMode === "speak" && styles.modeBtnActive]}
                onPress={() => setSingleMode("speak")}>
            ...
            <View style={styles.voiceBox}>
              ...
              <SliderRow label="Speed" value={singleVoice.speed} min={0.7} max={1.2} step={0.1}
                hint="0.7 = slow · 1.0 = normal · 1.2 = fast"
                onChange={v => setSingleVoice(p => ({ ...p, speed: v }))}
                onReset={() => setSingleVoice(p => ({ ...p, speed: 1.0 }))} />
              <SliderRow label="Stability" value={singleVoice.stability} min={0} max={1} step={0.25}
                hint="Low = expressive · High = consistent"
                onChange={v => setSingleVoice(p => ({ ...p, stability: v }))}
                onReset={() => setSingleVoice(p => ({ ...p, stability: 0.5 }))} />
              <SliderRow label="Style" value={singleVoice.style} min={0} max={1} step={0.25}
                hint="Low = neutral · High = dramatic"
                onChange={v => setSingleVoice(p => ({ ...p, style: v }))}
                onReset={() => setSingleVoice(p => ({ ...p, style: 0.2 }))} />
            </View>
            ...
            <Pressable style={styles.primaryBtn} onPress={runSingle}>
              ...
              <Text style={styles.primaryBtnText}>
                {singleMode === "translate" ? "Translate + Generate Audio" : "Generate Audio"}
              </Text>
            </Pressable>
        ...
        {activeTab === "batch" && (
          <View style={styles.section}>
            ...
            {/* Mode */}
            <View style={styles.modeRow}>
              <Pressable style={[styles.modeBtn, batchMode === "translate" && styles.modeBtnActive]}
                onPress={() => setBatchMode("translate")}>
                <Text style={[styles.modeBtnText, batchMode === "translate" && styles.modeBtnTextActive]}>🌐 Translate</Text>
              </Pressable>
              <Pressable style={[styles.modeBtn, batchMode === "speak" && styles.modeBtnActive]}
                onPress={() => setBatchMode("speak")}>
            ...
            {batchMode === "translate" ? (
              <View style={styles.langPair}>
                ...
                <Pressable style={styles.swapBtn} onPress={() => {
                  const tmp = batchSourceLang; setBatchSourceLang(batchTargetLang); setBatchTargetLang(tmp);
                }}>
                  <Text style={styles.swapBtnText}>⇄</Text>
                </Pressable>
                <View style={styles.langGroup}>
            ...
            <View style={styles.voiceBox}>
              <Text style={styles.sectionLabel}>VOICE CONTROLS</Text>
              <SliderRow label="Speed" value={batchVoice.speed} min={0.7} max={1.2} step={0.1}
                hint="0.7 = slow · 1.0 = normal · 1.2 = fast"
                onChange={v => setBatchVoice(p => ({ ...p, speed: v }))}
                onReset={() => setBatchVoice(p => ({ ...p, speed: 1.0 }))} />
              <SliderRow label="Stability" value={batchVoice.stability} min={0} max={1} step={0.25}
                hint="Low = expressive · High = consistent"
                onChange={v => setBatchVoice(p => ({ ...p, stability: v }))}
                onReset={() => setBatchVoice(p => ({ ...p, stability: 0.5 }))} />
              <SliderRow label="Style" value={batchVoice.style} min={0} max={1} step={0.25}
                hint="Low = neutral · High = dramatic"
                onChange={v => setBatchVoice(p => ({ ...p, style: v }))}
                onReset={() => setBatchVoice(p => ({ ...p, style: 0.2 }))} />
            </View>
            ...
            <TextInput
              ...
              placeholder={"Paste your numbered script here, e.g:\n1-1: Welcome to the JLPT N5 Kanji Accelerator.\n1-2: In this video, you will be introduced to kanji..."}
              placeholderTextColor={COLORS.textDim}
            />
            <Text style={styles.charCount}>{lineCount} lines detected</Text>

            <Pressable style={[styles.primaryBtn, batchRunning && styles.primaryBtnDisabled]}
              onPress={runBatch} disabled={batchRunning}>
      ...
      {/* Language pickers */}
      <LangPickerModal visible={showSourcePicker} selected={sourceLang}
        onSelect={setSourceLang} onClose={() => setShowSourcePicker(false)} />
      <LangPickerModal visible={showTargetPicker} selected={targetLang}
        onSelect={setTargetLang} onClose={() => setShowTargetPicker(false)} />
      <LangPickerModal visible={showSpeakPicker} selected={speakLang}
        onSelect={setSpeakLang} onClose={() => setShowSpeakPicker(false)} />
      <LangPickerModal visible={showBatchSourcePicker} selected={batchSourceLang}
        onSelect={setBatchSourceLang} onClose={() => setShowBatchSourcePicker(false)} />
      <LangPickerModal visible={showBatchTargetPicker} selected={batchTargetLang}
        onSelect={setBatchTargetLang} onClose={() => setShowBatchTargetPicker(false)} />
      <LangPickerModal visible={showBatchSpeakPicker} selected={batchSpeakLang}
        onSelect={setBatchSpeakLang} onClose={() => setShowBatchSpeakPicker(false)} />
    </SafeAreaView>
  );
}
